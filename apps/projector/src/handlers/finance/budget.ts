...
function applyEvent(state: BudgetSnapshotState, type: string, payload: any): void {
  switch (type) {
    ...
    case 'CategoryAdded': {
      const c: Category = { id: payload.categoryId, name: payload.name, parentId: payload.parentId ?? null };
...
async function loadLatestSnapshot(client: Client, streamId: string): Promise<{ state: BudgetSnapshotState; version: number }> {
