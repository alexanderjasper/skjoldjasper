...
class SydjyskImporterTest(SimpleTestCase):
    ...
    def test_first_row_is_positive_salary_with_bank_id(self):
        ...
        # external_id is the Transaktions-ID in col 10, not the col 11 reference.
        self.assertEqual(row["external_id"], "8185184253")
        self.assertEqual(row["to_account"], "9740 0005415861")

    def test_external_id_prefers_col10_over_col11_reference(self):
        # Second fixture row has both a col 10 id and a col 11 reference;
        # the stable id must win so distinct rows never collide.
        with open(FIXTURES / "sydjysk_sample.csv", encoding="utf-8") as f:
            rows = list(parse(f))
        self.assertEqual(rows[1]["external_id"], "0335589872")

    def test_negative_danish_amount(self):
        csv = (
            "DET FAGLIGE HUS;DET FAGLIGE HUS;9740 0005415861;;-1.791,00;;;"
            "01-04-2026;01-04-2026;01-04-2026;0335589872;;;;\n"
        )
        row = next(parse(StringIO(csv)))
        self.assertEqual(row["amount"], Decimal("-1791.00"))

    def test_falls_back_when_external_id_missing(self):
        # col 10 (Transaktions-ID) and col 11 (reference) both empty → the
        # content hash is used; date comes from col 9 (dispositionsdato).
        csv = (
    ...
    def test_raw_holds_original_row(self):
        with open(FIXTURES / "sydjysk_sample.csv", encoding="utf-8") as f:
            row = next(parse(f))
        self.assertEqual(row["raw"][10], "8185184253")
